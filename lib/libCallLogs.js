...
const fnCenterList = (params) =>
  new Promise((resolve) => {
    const { center } = params;

    let qq = knex("centers").where("userid", center);
    qq.limit(1);
...
const fnGetCallList = (params) =>
  new Promise((resolve) => {
    try {
      (async () => {
        const centerList = await fnCenterList(params);
        ...
        const resList = await listCallLogs({ ...params, center_id });
